...
public class GameController implements GameListener {
    ...
    public GameController(ChessboardComponent view, Chessboard model) {
        ...
        this.model = model;
        shuffleCount=3;
        save = new Saving(8,8,score,Step,model.getGrid());
    ...
    public void onPlayerSwapChess() {
        ...
        if (model.checkOnePosition(point1) || model.checkOnePosition(point2)) {
            ...
            Step--;
            ChessGameFrame.setScore(String.valueOf(score));
            ChessGameFrame.setStep(String.valueOf(Step));
            System.out.println();

            save.set(model.getGrid(),selectedPoint.getRow(), selectedPoint.getCol(), selectedPoint2.getRow(), selectedPoint2.getCol(),score,Step);
    ...
    public void onPlayerNextStep() {
        ...
        if (!model.checkChessBoard() && model.countBlanks() == 0) {
            if(Step==0) {
                Runnable ranking = new Runnable() {
                    ...
                    public void run() {
                        client = new Client();
                        boolean fl=client.sendLine("Ranking"+" "+User.getName()+" "+String.valueOf(score));
                        //boolean fl=client.sendLine("Ranking A 60");
                        System.out.println(fl);
                    }
                };
                Thread thread = new Thread(ranking);
                thread.start();
            }
            if (Step == 0 && score<1000) {
                AudioPlayer.playBgm("lib/audio/bgm.wav").stop();
        ...
        if (model.checkChessBoard() && model.countBlanks() == 0) {
            ...
            save.set(model.getGrid(),-1,-1,-1, -1,score,Step);
    ...
    public void loadGameFromFile(String path) throws IOException {
        save.Saved(path,level);
    }

    public void setInit(int step,int score,int level) {
        this.score = score;
        this.Step=step;
        this.level=level;
        save = new Saving(8,8,score,Step,model.getGrid());
