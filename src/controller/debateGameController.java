 ...
 */
public class debateGameController implements GameListener {

    private Chessboard model;
    private debateChessboardComponent view;
    private BoardforGUI view2;
    ...
    public debateGameController(debateChessboardComponent view,BoardforGUI view2, Chessboard model) {
        this.view = view;
        this.view2 = view2;
        this.model = model;
        shuffleCount=3;
        ...
        //repaint刷新界面
        view.repaint();
        view2.registerController(this);
        //不是model应该是读入的
        view2.initiateChessComponent(model);
        view2.repaint();
        //初始化保存
    ...
    public void onPlayershuffleChess() {
        if(shuffleCount==0) {
            debateGameFrame.FailShuffle();
        }else {
            shuffleCount--;
            debateGameFrame.setShufflenum(String.valueOf(shuffleCount));
            model.changePieces();
    ...
    public void onPlayerSwapChess() {
        ...
        if (model.checkOnePosition(point1) || model.checkOnePosition(point2)) {
            ...
            debateGameFrame.setMyScore(String.valueOf(score));
            debateGameFrame.setMyStep(String.valueOf(Step));
            //System.out.println("ffffffffffffffff");
            /*for(int i=0;i<8;i++) {
                for(int j=0;j<8;j++)
                    System.out.printf("%s ",model.getCell(i,j).getPiece().getName());
                System.out.println();
            }*/
            selectedPoint = null;
            selectedPoint2 = null;
    ...
    public void onPlayerNextStep() {
        ...
        if (!model.checkChessBoard() && model.countBlanks() == 0) {
            ...
            if (Step == 0) {
                debateGameFrame.FailTheGame();
                return;
            } else {
                debateGameFrame.uselessAction();
                return;
            }
        }
        ...
        if (model.checkIfFallDown()) {
            ...
            model.fallDown();
        } else if(model.countBlanks() == 0&&!findAres.isDeadend(model.getGrid())){
            //这个地方，死局自动洗牌没有判断
            debateGameFrame.HinttoShufft();
            onPlayershuffleChess();
        }else{
            for(int j=0;j<Constant.CHESSBOARD_COL_SIZE.getNum();j++){
                for(int i=Constant.CHESSBOARD_ROW_SIZE.getNum()-1;i>=0;i--) {
                    int id=0;
                    if(model.getCell(i,j).getPiece().getName().equals(" ")) {
                        model.fillCell(i,j);
                        view.fallDownforinit(i, j,i*72,model.getCell(i,j).getPiece(),id);
                        id++;//System.out.println(i);
                    }
                }
            }
        }
