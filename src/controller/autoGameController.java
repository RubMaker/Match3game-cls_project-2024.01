 ...
 */
public class autoGameController implements GameListener {

    private Chessboard model;
    private autoChessboardComponent view;
    ...
    public autoGameController(autoChessboardComponent view, Chessboard model) {
        this.view = view;
        this.model = model;
        shuffleCount=3;

        playBackgroundMusic("D:\\Jaosn Liu\\JavaCode\\Project3\\lib\\audio\\儿歌 - 虫儿飞.wav");
        //Step=10;
    ...
    public void onPlayerSwapChess() {
        ...
        if (selectedPoint == null || selectedPoint2 == null) {
            ChessGameFrame.illegalSwap();
            return;
        }
        if (model.checkChessBoard()) {
            ChessGameFrame.stillNeedToEliminate();
            return;
        }
        ...
        if (model.checkOnePosition(point1) || model.checkOnePosition(point2)) {
            ...
            Step--;
            autoChessGameFrame.setScore(String.valueOf(score));
            autoChessGameFrame.setStep(String.valueOf(Step));
            System.out.println();
            selectedPoint = null;
            selectedPoint2 = null;
        } else {
        ...
        if (model.checkIfFallDown()) {
            ...
            for(int j=0;j<Constant.CHESSBOARD_COL_SIZE.getNum();j++){
                ...
                for(int i=Constant.CHESSBOARD_ROW_SIZE.getNum()-1;i>=0;i--){
                    ...
                    fallDownCells.add(new Falldownmodel(i,j,count*100,model.getCell(i,j).getPiece().getName()));
                }
            }
    ...
    public void startGame(){
        ...
        Thread swapThread= new Thread(() -> {
            for(int p=1;p<=10;p++){
                pastMove hint = findAres.getHint(model.getGrid());
                ChessboardPoint point1=new ChessboardPoint(hint.getX1(),hint.getY1());
                ChessboardPoint point2=new ChessboardPoint(hint.getX2(),hint.getY2());
                selectedPoint=point1;
                selectedPoint2=point2;
                ChessPiece chess1 = new ChessPiece(model.getGrid()[selectedPoint.getRow()][selectedPoint.getCol()].getPiece().getName());
                ChessPiece chess2 = new ChessPiece(model.getGrid()[selectedPoint2.getRow()][selectedPoint2.getCol()].getPiece().getName());
                ChessComponent component1=(ChessComponent)view.getGridComponentAt(point1).getComponent(0);
                ChessComponent component2=(ChessComponent)view.getGridComponentAt(point2).getComponent(0);
                ...
                if (model.checkOnePosition(point1) || model.checkOnePosition(point2)) {
                    ...
                    try {
                        view.swapTwoChess(selectedPoint.getRow(), selectedPoint.getCol(), selectedPoint2.getRow(), selectedPoint2.getCol(), chess1, chess2, pointsToRemove).join();
