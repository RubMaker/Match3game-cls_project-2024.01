...
public class autoChessboardComponent extends JComponent {
    ...
    private autoGameController gameController;

    public autoGameController getGameController() {return gameController;}
    public autoChessboardComponent(int chessSize) {
    ...
    public void registerController(autoGameController gameController) {
    ...
    public Thread fallDown(int x,int y,int dis,ChessPiece chess) {
        int X=x+dis/72;
        int Y=y;
        //将目标格子移动的x,y
        //清空xy
    ...
    public Thread move(int x,int y,int op,ChessPiece chess,ChessPiece newChess,List<CellLocation> cells) {
        Runnable updateAComponent = new Runnable() {
            public void run() {
                if(chess==null) return;
                int xx=0,yy=0,dx=0,dy=0;
    ...
    public Thread move(int x,int y,int op,ChessPiece chess,ChessPiece newChess) {
        Runnable updateAComponent = new Runnable() {
            public void run() {
                if(chess==null) return;
                int xx=0,yy=0,dx=0,dy=0;
    ...
    //交换+删除操作
    public Thread swapTwoChess(int x, int y, int xx, int yy, ChessPiece chess, ChessPiece chess2, List<CellLocation> removeCell) {
        System.out.println("working");
        AtomicInteger opt = new AtomicInteger(0);
        if (xx > x) opt.set(1);
        if (xx < x) opt.set(-1);
        if (yy > y) opt.set(2);
        if (yy < y) opt.set(-2);
        ...
        Thread thread = new Thread(() -> {
            move(x, y, opt.get(), chess, chess2).start();
            move(xx, yy, -opt.get(), chess2, chess, removeCell).start();
        });
        thread.start();
        return thread;
    }
