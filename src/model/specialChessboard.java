 ...
 */
public class specialChessboard {
    private Cell[][] grid;


    public specialChessboard(int randomSeed) {
        this.grid =
    ...
    public specialChessboard(Cell[][] grid) {
        this.grid = grid;
    }
    ...
    public boolean checkIfFallDown(){
        for (int i = 0; i < Constant.CHESSBOARD_COL_SIZE.getNum(); i++) {
            for (int j = Constant.CHESSBOARD_ROW_SIZE.getNum()-2; j >=0; j--) {
                if(grid[j+1][i].getPiece().getName().equals(" ") && !grid[j][i].getPiece().getName().equals(" ") && !grid[j][i].getPiece().getName().equals("☠")){
                    return true;
                }
            }
        }
    ...
    //获取交换后需要消去的点的坐标（连成五个会消去整行、列）
    public List<CellLocation> pointsToRemove(int x1,int y1,int x2,int y2){
        List<CellLocation> points=new ArrayList<>();
        if(this.checkLeft(new ChessboardPoint(x1,y1))+this.checkRight(new ChessboardPoint(x1,y1))>=4){
            for(int i=0;i<Constant.CHESSBOARD_COL_SIZE.getNum();i++){
                points.add(new CellLocation(i,y1));
            }
        }
        else if(this.checkLeft(new ChessboardPoint(x1,y1))+this.checkRight(new ChessboardPoint(x1,y1))>=2){
            for(int i=x1-this.checkLeft(new ChessboardPoint(x1,y1));i<=this.checkRight(new ChessboardPoint(x1,y1))+x1;i++){
                points.add(new CellLocation(i,y1));
            }
        }

        if(this.checkDown(new ChessboardPoint(x1,y1))+this.checkUp(new ChessboardPoint(x1,y1))>=4){
            for(int i=0;i<Constant.CHESSBOARD_COL_SIZE.getNum();i++){
                points.add(new CellLocation(x1,i));
            }
        }
        else if(this.checkDown(new ChessboardPoint(x1,y1))+this.checkUp(new ChessboardPoint(x1,y1))>=2){
            for(int i=y1-this.checkUp(new ChessboardPoint(x1,y1));i<=this.checkDown(new ChessboardPoint(x1,y1))+y1;i++){
                points.add(new CellLocation(x1,i));
            }
        }

        if(this.checkLeft(new ChessboardPoint(x2,y2))+this.checkRight(new ChessboardPoint(x2,y2))>=4){
            for(int i=0;i<Constant.CHESSBOARD_COL_SIZE.getNum();i++){
                points.add(new CellLocation(i,y2));
            }
        }
        else if(this.checkLeft(new ChessboardPoint(x2,y2))+this.checkRight(new ChessboardPoint(x2,y2))>=2){
            for(int i=x2-this.checkLeft(new ChessboardPoint(x2,y2));i<=this.checkRight(new ChessboardPoint(x2,y2))+x2;i++){
                points.add(new CellLocation(i,y2));
            }
        }

        if(this.checkDown(new ChessboardPoint(x2,y2))+this.checkUp(new ChessboardPoint(x2,y2))>=4){
            for(int i=0;i<Constant.CHESSBOARD_COL_SIZE.getNum();i++){
                points.add(new CellLocation(x2,i));
            }
        }
        else if(this.checkDown(new ChessboardPoint(x2,y2))+this.checkUp(new ChessboardPoint(x2,y2))>=2){
            for(int i=y2-this.checkUp(new ChessboardPoint(x2,y2));i<=this.checkDown(new ChessboardPoint(x2,y2))+y2;i++){
                points.add(new CellLocation(x2,i));
            }
        }
        return points;
    ...
    public void removeTwoPoints(int x1,int y1,int x2,int y2){
        List<CellLocation> newList=this.pointsToRemove(x1,y1,x2,y2);
        for(int i=0;i<newList.size();i++){
            grid[newList.get(i).getX()][newList.get(i).getY()].removePiece();
        }
    }

    public void eliminateRow(int row){
        for(int i=0;i<Constant.CHESSBOARD_COL_SIZE.getNum();i++){
            grid[row][i].removePiece();
        }
    }
    public void eliminateCol(int col){
        for(int i=0;i<Constant.CHESSBOARD_ROW_SIZE.getNum();i++){
            grid[i][col].removePiece();
        }
    }
}
